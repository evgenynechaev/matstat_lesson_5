...
def task_4():
    ...
    print(f"        Не смещенная оценка дисперсии: {np.var(x_list, ddof = 1):.4f}")
    print(f"        Среднее квадратичное отклонение: {np.std(x_list, ddof = 1):.4f}")
    t_a2 = 3.250  # табличное значение для доверительного интервала 99% при n = 10, alfa = 0.01/2 = 0.005
    print(f"    табличное значение для доверительного интервала 99% при n = {n}: {t_a2:.4f}")
    delta = t_a2 * sigma / (n ** 0.5)
